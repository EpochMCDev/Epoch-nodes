...
const Nodes = {
    ...
    // dynmap hook (unused in squaremap build)
    dynmap: undefined,
    ...
    initialize: (options, callback) => {
        ...
        let mapGlue = options.mapGlue;
        Nodes.renderEditor();
        Nodes.mapGlue = mapGlue;

        // add custom hook for mouse move
        mapGlue.map.on("mousemove", Nodes.handleMouseMove);

        // create custom map pane and layer for nodes
        let customOverlayPane = mapGlue.map.createPane("nodesPane");
        customOverlayPane.id = "nodes-svg-renderer";

        let layerNodesRenderer = new NodesSvgRenderer(Nodes.setMapTransform, {});
        layerNodesRenderer.addTo(mapGlue.map);
    ...
    renderWorld: () => {
        const props = {
            ...
            map: Nodes.mapGlue.map,
            toLatLng: Nodes.mapGlue.toLatLng,
    ...
    handleMouseMove: (event) => {
        if ( !Nodes.mapGlue?.map ) {
            return;
        }

        // update cursor (block coords)
        const loc = Nodes.mapGlue.toBlock(event.latlng);
        Nodes.cursorX = loc.x;
        Nodes.cursorY = loc.z;
    ...
    */
    ...
    _getLatLngFromCoord: (x, z) => {
        ...
        if ( Nodes.mapGlue !== undefined ) {
            const latlng = Nodes.mapGlue.toLatLng(x, z);
            return Nodes.mapGlue.map.latLngToLayerPoint(latlng);
        }
    ...
    _togglePainting: () => {
        ...
        // squaremap fork: also re-render editor so paint button pressed state updates
        Nodes.renderEditor();
        Nodes.renderWorld();
    },
