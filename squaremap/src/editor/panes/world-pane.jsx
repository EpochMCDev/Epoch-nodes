    ...
    return (
        <div id="nodes-editor-resource-tooltip">
            ...
            <div className="nodes-editor-resource-tooltip-property">
                ...
                {
                    incomeList.length === 0 ?
                    <div className="nodes-editor-resource-tooltip-property-none">无</div>
                    : (null)
                }
            </div>
            {
...
export const WorldPane = (props) => {
    ...
    const selectedTownName = `城镇: ${town ? town.name : ""}`;
    const selectedTownNation = `国家: ${town !== undefined && town.nation !== undefined ? town.nation : ""}`;
    const selectedTownTerritories = `领土: ${town ? town.territories.length : ""}`;
    const selectedTownPlayersCount = `玩家: ${town ? town.residents.length : ""}`;
    ...
    const selectedTerritoryName = `名称: ${selectedTerritory !== undefined ? selectedTerritory.name : ""}`;
    const selectedTerritoryId = `ID: ${selectedTerritory ? selectedTerritory.id : ""}`;
    const selectedTerritoryCore = `核心: ${selectedTerritory && selectedTerritory.coreChunk ? `${selectedTerritory.coreChunk.x},${selectedTerritory.coreChunk.y}` : ""}`
    const selectedTerritorySize = `区块数: ${selectedTerritory ? selectedTerritory.size : ""}`;
    ...
    return (
        <>
        <div id="nodes-editor-options-header">地图选项:</div>
        ...
        <div className="nodes-editor-help">
            <div>操作说明:</div>
            <div>- 右键点击选择领土以查看信息</div>
            <div>- 鼠标悬停资源节点列表查看详情</div>
        </div>
